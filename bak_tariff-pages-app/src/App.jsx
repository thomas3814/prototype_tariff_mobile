...
function findCountryInfo(countryItems, country) {
  if (!country) {
    return null;
  }

  return (countryItems ?? []).find((item) => item.country === country) ?? null;
}
...
function normalizeFiltersForSnapshot(snapshot, currentFilters) {
  ...
  const availableImportCountries = product.availableImportCountries ?? [];
  const availableExportCountries = product.availableExportCountries ?? [];

  const validImportContinents = new Set(availableImportCountries.map((item) => item.continent));
  if (nextFilters.importContinent && !validImportContinents.has(nextFilters.importContinent)) {
    nextFilters.importContinent = '';
    nextFilters.importCountry = '';
  }

  const validExportContinents = new Set(availableExportCountries.map((item) => item.continent));
  if (nextFilters.exportContinent && !validExportContinents.has(nextFilters.exportContinent)) {
    nextFilters.exportContinent = '';
    nextFilters.exportCountry = '';
  }

  const importCountryInfo = findCountryInfo(availableImportCountries, nextFilters.importCountry);
  if (nextFilters.importCountry) {
    if (!importCountryInfo || nextFilters.importCountry === nextFilters.exportCountry) {
      nextFilters.importCountry = '';
    } else {
      nextFilters.importContinent = importCountryInfo.continent ?? nextFilters.importContinent;
    }
  }

  const exportCountryInfo = findCountryInfo(availableExportCountries, nextFilters.exportCountry);
  if (nextFilters.exportCountry) {
    if (!exportCountryInfo || nextFilters.exportCountry === nextFilters.importCountry) {
      nextFilters.exportCountry = '';
    } else {
      nextFilters.exportContinent = exportCountryInfo.continent ?? nextFilters.exportContinent;
    }
  }

  const validImportCountries = new Set(
    availableImportCountries
      .filter((item) => item.country !== nextFilters.exportCountry)
  ...
  if (!nextFilters.importCountry && nextFilters.importContinent) {
    const hasCountryForContinent = availableImportCountries.some(
      (item) => item.continent === nextFilters.importContinent && item.country !== nextFilters.exportCountry,
    );

    if (!hasCountryForContinent) {
      nextFilters.importContinent = '';
    }
  }
  ...
  const validExportCountries = new Set(
    availableExportCountries
      .filter((item) => item.country !== nextFilters.importCountry)
  ...
  if (!nextFilters.exportCountry && nextFilters.exportContinent) {
    const hasCountryForContinent = availableExportCountries.some(
      (item) => item.continent === nextFilters.exportContinent && item.country !== nextFilters.importCountry,
    );
...
export default function App() {
  ...
  function handleImportContinentChange(importContinent) {
    setFilters((current) => {
      ...
      const importCountryInfo = findCountryInfo(selectedProduct.availableImportCountries, current.importCountry);
      const shouldKeepCountry = !current.importCountry
        || !importContinent
        || importCountryInfo?.continent === importContinent;

      return {
        ...current,
        importContinent,
        importCountry: shouldKeepCountry ? current.importCountry : '',
      };
    });
  }
  ...
  function handleImportCountryChange(importCountry) {
    setFilters((current) => {
      ...
      if (importCountry === current.exportCountry) {
        return current;
      }

      const importCountryInfo = findCountryInfo(selectedProduct.availableImportCountries, importCountry);

      if (!importCountryInfo) {
        return {
          ...current,
          importCountry: '',
          importContinent: '',
      ...
      return {
        ...current,
        importCountry,
        importContinent: importCountryInfo.continent ?? current.importContinent,
      };
    });
  }
  ...
  function handleExportContinentChange(exportContinent) {
    setFilters((current) => {
      ...
      const exportCountryInfo = findCountryInfo(selectedProduct.availableExportCountries, current.exportCountry);
      const shouldKeepCountry = !current.exportCountry
        || !exportContinent
        || exportCountryInfo?.continent === exportContinent;

      return {
        ...current,
        exportContinent,
        exportCountry: shouldKeepCountry ? current.exportCountry : '',
      };
    });
  }
    ...
    setFilters((current) => {
      ...
      if (!exportCountry) {
        return {
          ...current,
          exportCountry: '',
          exportContinent: current.exportContinent,
        };
      }

      if (exportCountry === current.importCountry) {
        return current;
      }

      const exportCountryInfo = findCountryInfo(selectedProduct.availableExportCountries, exportCountry);

      if (!exportCountryInfo) {
        return {
          ...current,
          exportCountry: '',
          exportContinent: '',
      ...
      return {
        ...current,
        exportCountry,
        exportContinent: exportCountryInfo.continent ?? current.exportContinent,
      };
    });
  }
