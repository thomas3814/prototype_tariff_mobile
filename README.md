# prototype_tariff_mobile 소스 패키지 (v17)

> 이 ZIP은 `prototype_tariff_mobile` 프로젝트의 **v17 소스 전달용 묶음**입니다.
> 주요 변경은 **모바일 모드에서 `수출국 기준 수입국 비교` case도 가로 bar 목록형으로 통일**한 것입니다.

## v17 핵심 변경

- 모바일 모드에서 `수입국 기준 수출국 비교`와 `수출국 기준 수입국 비교` 모두 동일한 가로 bar 요약형을 사용
- 모바일 목록은 `국가명 / bar / %`만 우선 표시
- 국가 터치 시에만 `원산지 기준`, `HS`, `적용관세 / 협정관세 / 기본관세` 상세 노출
- PC 모드는 유지
